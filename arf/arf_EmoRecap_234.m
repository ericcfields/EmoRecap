%Artifact rejection script for EmoRecap ERP
%AUTHOR: Eric Fields
...
EEG  = NCL_pop_artstep(EEG, 'Channel', [1:num_chans, biEOG_chan], 'Flag', [1, 3], 'Threshold', step_thresh, 'Twindow', rej_window, 'Windowsize', step_windowsize, 'Windowstep', step_windowstep, 'Review', 'off');
...
fprintf('\nEncoding rejection rate: %.1f%%\n', sum(rej_by_bin(1:6))/sum(EEG.EVENTLIST.trialsperbin(1:6)) * 100)
fprintf('Retrieval rejection rate: %.1f%%\n', sum(rej_by_bin(10:15))/sum(EEG.EVENTLIST.trialsperbin(10:15)) * 100)
fprintf('Smallest bin (1-6 and 10-15): %d\n\n', min(EEG.EVENTLIST.trialsperbin([1:6, 10:15]) - rej_by_bin([1:6,10:15])));
